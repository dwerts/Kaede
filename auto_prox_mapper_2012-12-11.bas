rem bke 2012-12-11 updated to map near a probe changer
...
rem bke 2012-12-11 requires version 1212.11 or greater
num_probe_ports = ?
if (num_probe_ports > 0)
	dim probe_porty[num_probe_ports]
	min_port_y = 999.9
	for i = 0 to num_probe_ports - 1
		probe_porty[i] = ?
		min_port_y = minf(min_port_y, probe_porty[i])
	next i
	if (min_port_y > 0.0)
		ymax = int(min_port_y - 57.0)
	endif
endif
...
display("make sure the probe is clear to move to the y+ starting location")
move("y", ymax - ref[1])
