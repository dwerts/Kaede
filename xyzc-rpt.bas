file = "c:\pecos\status\38-xyzc-rpt-10000.txt"
num_repeats = 1000
backoff_dist = 5
c_backoff_dist = 3


logmsg(file, "Starting all axis repeatability test - " + str(num_repeats) + " cycles")
...
for i = 1 to num_repeats
rem    qbegin(1)
rem    qdecel()
    display_status("moving in")
    move("xyzc", axs_pos_tch[0], axs_pos_tch[1], axs_pos_tch[2], axs_pos_tch[3])
rem    qgo()
